...
export async function middleware(req) {
  ...
  if (!cookie) {
    return NextResponse.json({message: "Invalid login token"}, {status: 401});
  }
...
export const config = {
  matcher: ["/api/user/:path*", "/api/category/categoryCrud/:path*", "/api/products/add_product/:path*" , "/api/add_to_cart/:path*"],
};
