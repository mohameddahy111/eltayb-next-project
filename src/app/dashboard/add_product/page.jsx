...
export default function page() {
  ...
  function chengePriceList(e) {
    ...
    if (e.target.name === 'value_of_offer' || e.target.name === 'value') {
      ...
      price.final_price = price.value - (price.value * price.value_of_offer / 100)
      setPrice({...price})
      console.log(price)
      return
    }
  ...
  function addPrice() {
    ...
    if (isExist) {
      enqueueSnackbar(`${price.size} is Exist in your list`, {variant: 'error'})
      return
    }
    if (price.value === 0) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      // })
      await axios.post("products/add_product", newProduct).then((res) => {
        console.log(res)
        if(res.status === 200){
         ...
         <Grid item md={4} xs={12} position={'relative'}>
           <Box p={2} width={'100%'} sx={{border: '2px dashed #09c'}}>
             <Box hidden={!addImage && !urlImg}>
               <img src={addImage? URL.createObjectURL(addImage) :urlImg } alt={'img'} width={'100%'} height={'100%'}/>
             </Box>
             <Box className={'box-flex-center'}>
               <IconButton>
                 <label htmlFor={'addImg'}><Add/> </label>
               </IconButton>
               <input id="addImg" name='addImg' type="file" hidden onChange={(e) => getImg(e)}/>
               <Typography px={2}>
                 or
               </Typography>
           ...
           <Grid container spacing={1}>
             ...
             <Grid item md={12} xs={12}>
               <Paper elevation={0} sx={{padding: '10px'}}>
                 <Typography component={'h6'} variant={'h6'} textTransform={'capitalize'} py={3}>
                   add price
                 </Typography>
                 <Grid container spacing={1}>
                   <Grid item md={6} xs={12}>
                     ...
                     <TextField
                      ...
                      InputProps={{
                        endAdornment: <InputAdornment
                         position="end">{price.size.includes('Liter') ? "Bottle" :price.size.includes('Kg') ? "Kg": "Caps"}  </InputAdornment>,
                      }}
                     />
                   </Grid>
                   <Grid item md={12} xs={12} display={'flex'} justifyContent={'space-around'} alignItems={'center'}>
                   ...
                   <Grid item md={12} xs={12}>
                     <TextField
                      label="Final Price"
                      value={price.offer ? (price.value - price.value * price.value_of_offer / 100) : price.value}
                      name='final_price'
                      fullWidth
                      InputProps={{
                        endAdornment: <InputAdornment position="end">EP</InputAdornment>,
                 ...
                 <Grid item md={12} xs={12} py={2}>
                   <Button variant="contained" color="primary" onClick={addPrice}>
                     add price
                   </Button>
                 </Grid>
